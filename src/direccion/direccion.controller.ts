...
export class DireccionesController {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('mia')
  async obtenerMiDireccion(@Req() req: any) {
    const usuarioId = req.user.id;

    const direccion = await this.direccionesService.obtenerDireccionPorUsuario(usuarioId);

    if (!direccion) {
      throw new NotFoundException('No se encontró dirección para este usuario');
    }
...
async obtenerDireccionPorCorreo(@Param('correo') correo: string) {
  const direccion = await this.direccionesService.obtenerDireccionPorCorreo(correo);
  if (!direccion) {
    throw new NotFoundException('No se encontró dirección para este correo');
  }
  return direccion;
}
}